...
**One last look at the wire.** The interrupt line is an *asynchronous* signal from a device with its own clock, landing in a CPU that samples on the beat — exactly the danger studied in **Clock Domains and Metastability** *(not yet released)*. Real interrupt inputs pass through synchronizer flip-flops before the flag is trusted. Even the doorbell needs debouncing.
...
- **Kindred:** **Clock Domains and Metastability** *(not yet released)* — the interrupt line is an asynchronous signal entering a clocked world, and it is synchronized like one; [[Pipelining and Simultaneous Multithreading]] — what a context switch really disturbs.
