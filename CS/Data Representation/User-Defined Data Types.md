...
A record is a *layout contract*: field offsets fixed at definition time, so `booking.taxi` compiles to "the bytes starting 40 after wherever this record starts" — no search, no lookup, one addition. That is why dot access is free while a dictionary lookup pays for hashing (**Hash Tables** *(not yet released)*'s price list), and why the compiler must know every field's type up front: it is computing the offsets.
...
- **Siblings in the taxonomy:** [[Stacks and Queues]], **Binary Trees** *(not yet released)*, **Hash Tables** *(not yet released)*, **Graphs** *(not yet released)* — all formally composite user-defined types per the pseudocode guide; each bay card is a design worked out in full.
