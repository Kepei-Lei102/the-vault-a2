...
One adder, both operations. This is why two's complement won: a CPU needs **no subtractor** — to compute $a-b$ it inverts $b$, feeds a 1 into the adder's carry-in (that's the "+1"), and adds. The **full-adder** *(not yet released)* you'd build from gates is the whole arithmetic unit for signed *and* unsigned numbers at once.
...
- **The circuit:** **Half-Adder and Full-Adder** *(not yet released)* — the full-adder built from gates is the one unit that does signed and unsigned add/subtract, *because* of two's complement.
