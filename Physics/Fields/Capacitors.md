...
- **The application (cross-domain):** [[RAM and the Memory Hierarchy]] — one capacitor holds one **DRAM** bit, and its $RC$ leak is *why* DRAM refreshes; **Clock Domains and Metastability** *(not yet released)* — $RC$ timing behind ring-oscillator clocks; [[How a Chip Is Made]] — the trench/stacked capacitor etched into silicon.
