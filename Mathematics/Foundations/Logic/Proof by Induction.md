...
> Recall that induction climbs $\mathbb{N}$ along the $+1$ steps every integer is built from. But *any* recursively built world supports the same argument: prove the claim for the atoms, prove each construction rule preserves it, and the claim holds for everything constructible. That is **structural induction** — induction on shape rather than size — and it is how one proves facts about [[Recursion]] (a recursive function's correctness proof *is* an induction on its argument — the twin claim, made from the other side), about **Binary Trees** *(not yet released)* (every non-empty tree has one more node than edge), about every grammar written in BNF ([[Compilers and Interpreters]]). When a computer scientist says "by induction on the structure of the tree," this card is what they mean.
